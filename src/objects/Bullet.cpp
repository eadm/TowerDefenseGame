...
bool Bullet::isReached() {
    ...
    return (std::abs(c_pos.x - t_pos.x) <= tileW / 2) && (std::abs(c_pos.y - t_pos.y) <= tileH / 2);
}
...
void Bullet::calculateSpeed() {
    ...
    time_to_target = (float)getDistance(t_pos) / velocity;
...
float Bullet::getAlpha(Vector2f aim) {
    ...
    float tmp_aplpha = atan2( aim.x - c_pos.x, c_pos.y - aim.y ) * 180 / PI;
